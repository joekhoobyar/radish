...
module TypeCast
  ...
  module Object
    ...
    #
    def cast_to(klass)
      (klass === self) ? self : klass.cast_from(self)
    end
  end
  ...
  module Class
    ...
    def cast_from(object)
      ...
      method_to = "to_#{self.name.underscore}".to_sym
      if object.respond_to? method_to
        retval = object.send(method_to)
        return retval
      end
      method_from = "from_#{object.class.name.underscore.gsub('/', '__')}".to_sym
...
class Class #:nodoc:
  ...
  def self.from_string(string)
    string = string.to_s.gsub(/__(.?)/, '::\1').classify
